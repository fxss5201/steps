...
;(function(undefined) {
    ...
    _steps.prototype = {
        ...
        _initial: function(options) {
            ...
            // 如果未设置props中的排序，则需要获得默认排序
            if(_this.options.dataOrder.length == 0){
                _this.getDataOrder();
            }
            _this.render();
        },
        getDataOrder: function(){ 
            ...
            for(var item in options.props){
                options.dataOrder.push(options.props[item]);
            }
            console.log(this)
        },
        ...
        render: function(){ // 渲染样式
            ...
            // 添加最外层的class样式
            parentNode.className = parentNode.className + options.customClass;
